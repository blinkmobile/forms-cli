...
const fileHelper = require('./file-list-helper')
...
function makeTemplateHash (templatePath) {
  return fileHelper.getFolderList(`${templatePath}/**/`).then((folders) => {
    const hash = {}
    const regex = new RegExp(`^\/?${templatePath}\/?$`, 'i') //eslint-disable-line
...
module.exports = {
  load: (templatePath) => makeTemplateHash(templatePath).then(loadAllTemplates), // .then(() => templatePath),
