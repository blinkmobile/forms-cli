...
function processForm (options) {
  ...
  console.log('----------------------------------------------')
  console.log('processing definition, form name: ' + formName)

    const interactionNames = (f) => {
      console.log(`get interaction names for ${formName} - ${JSON.stringify(f)} `)
      return (f[0] || formName + f[1].action)
    }
    const fl = (varName) => {
      console.log(`flattening ${formName} - ${varName}`)
      return flatten(form, varName, flattenOptions)
    }
...
console.log(`addAngularConfig => ${JSON.stringify(formData)}`)
...
console.log('normalised data for ' + formName)
